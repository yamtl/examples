...
class MultipleSourcesTest extends YAMTLModule {
	final BASE_PATH = 'model'

	@Test
	def void testMultipleSources() {		
		...
		def xform = new MultipleSources(srcRes.contents[0], tgtRes.contents[0])
		xform.loadInputModels(['in': BASE_PATH + '/wakeup.xmi'])
		xform.execute()
		xform.saveOutputModels(['out': BASE_PATH + '/multipleSourcesOutput.xmi'])
		...
		def expectedResource = xform.loadModel(BASE_PATH + '/multipleSourcesExpectedOutput.xmi', false)
