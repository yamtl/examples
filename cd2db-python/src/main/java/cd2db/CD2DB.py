...
def unmarshall(obj):
    if isinstance(obj, list):
        # If obj is a list
        return [item.__dict__['_dynamic_eobject'] for item in obj]
    else:
        # If obj is an object
        return obj.__dict__['_dynamic_eobject']
...
class ClassToTable_t_action(Action):
    def run(self):
        ...
        # FIXME: appending to lists get(listReference).add()
        # key.owner = unmarshall(t)
        # t.col.append(unmarshall(key))
        # t.key = unmarshall(key)
        
class ClassToTable_key_action(Action):
    def run(self):
        c = self.module.c()
        key = self.module.key()
        
        key.name = "objectId"
        # key.type = self.module.fetch(self.module.fetch("objectIdType"))
...
class MultiValuedDataTypeAttribute2Column_id_action(Action):
    def run (self):
        ...
        #FIXME: define helper
        # id.type = self.module.fetch(self.module.fetch("objectIdType"))
...
class ClassAttribute2Column_col_action(Action):
    def run(self):
        att = self.module.att()
        col = self.module.col()
        col.name = att.name + "Id"
        # col.type = self.module.fetch(self.module.fetch('objectIdType'))

class MultiValuedClassAttribute2Column_t_action(Action):
    def run (self):
        att = self.module.att()
        t = self.module.t()
        id = self.module.id()
...
class MultiValuedClassAttribute2Column_id_action(Action):
    def run (self):
        att = self.module.att()
        id = self.module.id()
        id.name = att.owner.name + "Id" # FIXME: lower case in first letter
...
class MultiValuedClassAttribute2Column_col_action(Action):
    def run (self):
        att = self.module.att()
        col = self.module.col()
        col.name = att.name + "Id"
...
class CD2DB(YAMTLModule):
    ...
    def __init__(self, CD, DB):
        ...
        self.ruleStore([
            rule('ClassToTable')
                ...
                .out('key', DB.getEClassifier('Column'), ClassToTable_key_action(self)),
            ...
            rule("ClassAttribute2Column")
                ...
                .out("col", DB.getEClassifier("Column"), ClassAttribute2Column_col_action(self)),
            ...
            rule("MultiValuedClassAttribute2Column")
                ...
                .out("t", DB.getEClassifier("Table"), MultiValuedClassAttribute2Column_t_action(self))         
                .out("id", DB.getEClassifier("Column"), MultiValuedClassAttribute2Column_id_action(self))                
                .out("col", DB.getEClassifier("Column"), MultiValuedClassAttribute2Column_col_action(self)),                
        ])
    ...
    def key(self):
        return ExtendedDynamicEObject(self.fetch('key'))
    def t(self):
