			...
			new Rule('StateMachine_2_DeploymentBehavior')
				...
				.out('out', DEP.deploymentBehavior,
					[ 
						...
						// Transform states
						val behaviorStates = stateMachine.states.fetch as List<BehaviorState>
						out.states += behaviorStates 
						
						// Transform transitions
						var behaviorTransitions = new ArrayList<BehaviorTransition>
						for (state : stateMachine.states) {
							// TODO: check if filter needs to be applied
							behaviorTransitions += 
								state.outgoingTransitions.fetch as List<BehaviorTransition>
						}
			...
			new Rule('State_2_BehaviorState')
				...
				.out('out', DEP.behaviorState,
					[ 
						...
						out.outgoing += state.outgoingTransitions.fetch as List<BehaviorTransition> 
					]
				).build()
				.build(),
